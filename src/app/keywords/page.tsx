...
import { reviewKeyword, reviewTarget, addKeyword, deleteKeyword, suggestForTarget } from './actions';
...
async function load(params: { view?: string; q?: string; page?: string }) {
  ...
  const groups = view === 'risk' || view === 'conflict' ? [] : await db<GroupRow[]>`
    with live as (
  ...
  return { summary, risks, groups, view, q, page };
}
