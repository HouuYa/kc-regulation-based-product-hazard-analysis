...
export const CALL_SITES: CallSite[] = [
  ...
  },
  {
    ...
    goal: '서술에 맞는 HF(위해요인)·DT(사고유형) 코드를 고른다 (068부터: 사고사진 hazard_note가 있으면 서술 뒤에 덧붙여 함께 준다 — evidence_span이 그 블록에서 나오면 원문 인용이 아니므로 검수 시 구분해야 한다)',
  ...
  },
  {
    ...
    stage: '분석 실행',
    name: '보고서가 한 일 추출 (병행 점검)',
    when: '사고보고서 병행 점검을 실행할 때',
    goal: '보고서가 어떤 시험을 했고 동일성·결론을 무엇이라 적었는지를 인용해 온다. 원인을 묻지 않는다 — 코드는 규칙과 통계가 따로 붙인다',
    modelEnv: 'OPENAI_BULK_MODEL → (흔들리면) OPENAI_ESCALATE_MODEL',
    schema: 'investigation_extract',
    enumLock: '구역·판정을 enum 으로 고정. 시험명은 기준마다 표기가 달라 enum 으로 못박을 수 없어, 대신 evidence_span 부분문자열 검사로 창작을 막는다',
    evidence: 'evidence_span (원문 인용, 필수 + 저장 직전 부분문자열 재검사)',
    storedAt: 'case_investigation_item.evidence_span · span_verified',
    screen: '분석 결과 (/analysis — 「보고서가 한 일」)',
  },
  {
    purpose: 'vision',
    stage: '위해요인 코드 부여',
    ...
    storedAt: 'source_file_image (사진마다 한 행 — 068 이전에는 여기 적힌 JSON 칸으로 잘못 적혀 있었다)',
    screen: '사고보고서 (업로드 시 추출·저장) · 분석 결과 (/analysis, 068)',
  },
  {
