...
export function tagClause(
  ...
  /*
    코드 목록을 앞에, 대상을 뒤에 두는 순서를 지킨다 (2026-09-08 확인)

    이 순서는 원래 읽기 좋으라고 정한 것인데, 캐시 단가에서도 맞는 순서다. 코드 목록
    2,500여 토큰이 호출마다 글자 그대로 같으므로 그 구간이 1/10 단가로 청구된다.
    대상 조항을 앞으로 옮기면 그 이득이 통째로 사라진다 — 순서를 바꾸지 말 것.
  */
  const user = [
    '[위해요인(HF) 코드 목록]',
