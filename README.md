...
> **처음 보신다면 → [전체 프로세스와 용어](docs/전체_프로세스와_용어.md)** — 무엇을 위해 어떤 순서로 도는지, 「검수」·「사전」처럼 여러 뜻으로 쓰이는 말을 하나로 맞춘 문서입니다. 화면마다 머리에 뜨는 업무 순서도의 원본이 이 문서의 [§3 전체 순서](docs/전체_프로세스와_용어.md#3-전체-순서)이고, 어느 화면이 어느 걸음을 맡는지는 그 아래 「화면이 각각 어느 걸음을 맡는가」 표에 있습니다. **화면과 이 문서가 어긋나면 문서를 먼저 고치고 화면을 맞춥니다.**
>
